...
console.log(data)
    ...
    if (caseTwo) io.to(`room:${diaryId}`).emit(today);
    ...
    io.to(`room:${diaryId}`).emit(newMessage); //채팅시 채팅방에 알림 생기게 프>론트에서 소켓 콜백함수를 조정해야함
  });
  ...
  socket.on('invited', async (data) => {
    ...
    */
    ...
    //다이어리에 이미 초대된 사람 있으면 해당 이벤트로 다이어리 이름 과 함께소켓발생
      ...
      socket.to(diaryId).emit(`invite:${invitedUserId}`, true , caseTwo||caseOne);
    }
  });
